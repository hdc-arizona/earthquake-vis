...
function sim(eid,numStory) {
    ...
    this.getDataMatrix = function(startTime=0, endTime=this.length, startFloor=0, endFloor=14) {
        result = [];
        var index = 0;
...
function updatePCABins(svg) {
    ...
    */
    ...
    for (var i = 0; i < matrix.length; i++) {
        var x = Math.floor((matrix[i][0] - pcaXExtent[0]) * N / (pcaXExtent[1] - pcaXExtent[0]));
        var y = Math.floor((matrix[i][1] - pcaYExtent[0]) * N / (pcaYExtent[1] - pcaYExtent[0]));
        x = Math.max(x, 0);
        x = Math.min(x, N-1);
        y = Math.max(y, 0);
        y = Math.min(y, N-1);
