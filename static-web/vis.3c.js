...
function drawHeatmap(i) {
    ...
    var colorScale = d3.scale.linear()
        .domain([maxDomain,1,0,-1,-maxDomain])
        .range(["#f03b20","#756bb1","#f7f7f7","#74c476","#253494"])
...
function prepareDensityMap() {
    ...
    for (var i = iStart; i <= iEnd; i++) {
        for (var j = jStart; j <= jEnd; j++) {
            var s = Math.floor((parseFloat(dataMatrix[i][j]) + 6) * N / F);
            var m = Math.floor((parseFloat(dataMatrix[i][j+F]) + 6) * N / F);
            var d = Math.floor((parseFloat(dataMatrix[i][j+2*F]) + 6) * N / F);
            sm[s][m]++;
            sd[s][d]++;
            md[m][d]++;
            //console.log([s,m,d])
